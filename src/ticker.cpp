...
static const unsigned long TRANSITION_PAUSE_MS  = 600UL; // ページ切替前の停止時間
...
//     → scrollX を毎ティック -SCROLL_SPEED_PX
//     → scrollX < -scrollTextW になったらスクロール完了:
//        ・ティッカーエリアをクリア
//        ・scrollPauseEndMs = now + TRANSITION_PAUSE_MS をセット
//        ・return (この呼び出しでは newsIndex を変えない)
//
//   Phase 2: ポーズ中 (scrollPauseEndMs != 0 かつ now < scrollPauseEndMs)
//     → 何もせず return
//
//   Phase 3: ポーズ完了 (scrollPauseEndMs != 0 かつ now >= scrollPauseEndMs)
//     → scrollPauseEndMs = 0 にクリア
//     → newsIndex を進めて redrawNewsContent()
//
//   この 3 フェーズ分離により、Mutex 取得が遅延しても
//   同一フレーム内での二重遷移が構造的に起きない。
// ================================================================
